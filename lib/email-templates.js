...
// ============================================================
// MIDNIGHT EXECUTIVE EMAIL DESIGN SYSTEM
// Exact layout, structure, and aesthetics:
// - Outer neutral canvas (#f0f1f5), dark midnight obsidian card (#060710)
// - SF Pro / Arial clean typography, high contrast white with periwinkle (#bdc3ff)
// - Dual-engine pill CTA button (VML Outlook <v:roundrect> + CSS gradient #282e5e to #5261b3)
// - Responsive layout grids (layout-0, layout-2 details, layout-3 bento, layout-5 footer)
// - Full cross-client compatibility (Gmail, Outlook 2016/2019/365, Apple Mail, iOS/Android)
// ============================================================
function plainLayout({ body, email, kicker = 'Adams X Protocol', title = 'Adams X Project' }) {
...
.l2-c0,.l2-c1,.l2-c2{display:block!important;width:100%!important}
.l2-s0,.l2-s1{display:block!important;width:auto!important;height:16px;font-size:0}
}
</style>
<!--<![endif]-->
<!--[if !mso]><!-->
...
.layout-3 .ebi-mw-426{max-width:426px!important}
.layout-3 .ebi-mw-426 img{width:100%!important}
}
</style>
<!--<![endif]-->
<!--[if !mso]><!-->
...
.l4-c0,.l4-c1,.l4-c2{display:block!important;width:100%!important}
.l4-s0,.l4-s1{display:block!important;width:auto!important;height:3px;font-size:0}
}
</style>
<!--<![endif]-->
<style>
@media(max-width:550px){.ers-fs-213{font-size:18.7px!important}}
</style>
</head>
      ...
      <table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0">
        <tr>
          <td align="left" style="vertical-align:middle;">
            <a href="https://adamsxproject.com.ng" target="_blank" style="text-decoration:none;display:inline-block;">
              <table role="presentation" cellspacing="0" cellpadding="0" border="0">
                <tr>
                  <td style="vertical-align:middle;padding-right:8px;">
                    <img src="https://adamsxproject.com.ng/favicon-32x32.png" width="20" height="20" alt="Logo" style="display:block;border-radius:4px;" />
                  </td>
...
${body}
  ...
  <tr>
    <td style="vertical-align:top">
      <table border="0" cellpadding="0" cellspacing="0" class="layout-5" align="center" style="display:table;border-spacing:0px;border-collapse:separate;width:100%;max-width:100%;table-layout:fixed;margin:0 auto;background-color:#000000;background:linear-gradient(180deg, #060710 0%, #000000 35%, #0a238a 80%, #bfccfc 100%);">
        <tbody><tr>
...
function signOff(name = '— Adams X') {
    return `      <p style="margin:24px 0 0;font-size:15px;font-weight:700;color:#FFFFFF;font-family:Arial,Helvetica,sans-serif;">${name}</p>\n`;
}

// Helper — PS line (Glass callout)
function ps(text) {
    return `      <table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0" style="margin:22px 0 0;background:#0d122b;border-left:3px solid #5261b3;border-radius:6px;">
        <tr>
          <td style="padding:14px 18px;">
            <p style="margin:0;font-size:13.5px;line-height:1.6;color:#CBD5E1;font-family:Arial,Helvetica,sans-serif;"><strong style="color:#bdc3ff;">P.S.</strong> ${text}</p>
          </td>
        </tr>
      </table>\n`;
}
...
function link(url, label) {
    return `<a href="${url}" target="_blank" rel="noopener" style="color:#bdc3ff;font-weight:600;text-decoration:underline;">${label || url}</a>`;
}

// Helper — Gold accent highlight
function gold(text) {
    return `<span style="color:#FFE600;font-weight:700;">${text}</span>`;
}

// Helper — Periwinkle accent highlight
function cyan(text) {
    return `<span style="color:#bdc3ff;font-weight:700;">${text}</span>`;
}
...
function sectionTitle(title, color = '#bdc3ff') {
    return `      <table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0" style="margin:22px 0 14px;">
        <tr>
          <td dir="ltr" class="ers-fs-213" style="color:${color};font-size:21.3px;font-weight:700;letter-spacing:-0.03em;text-align:center;line-height:1.35;mso-line-height-alt:29px;font-family:Arial,Helvetica,sans-serif;">
            ${title}
          </td>
        </tr>
      </table>\n`;
...
// Helper — Dual-Engine Pill CTA Button (VML Outlook + CSS)
// Usage: ctaButton(url, label) or ctaButton(url, label, true) for outline variant
// Legacy call format (label, url) auto-detected and handled
function ctaButton(urlOrLabel, labelOrUrl, isOutline = false) {
    // Auto-detect legacy (label, url) vs new (url, label) call format
    let url, label;
    if (urlOrLabel && urlOrLabel.startsWith('http')) {
        url = urlOrLabel;
        label = labelOrUrl || 'Get Access';
    } else {
        label = urlOrLabel || 'Get Access';
        url = labelOrUrl || '#';
    }
...
// Helper — Testimonial Box (midnight styled quote card)
function testimonialBox(quote, author) {
    return `      <table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0" style="background:#0d122b;border-left:4px solid #5261b3;border-radius:8px;margin:20px 0;overflow:hidden;">
        <tr>
          <td style="padding:20px 22px;">
            <p style="margin:0 0 12px;font-family:Arial,Helvetica,sans-serif;font-size:15px;line-height:1.7;color:#E2E8F0;font-style:italic;">&ldquo;${quote}&rdquo;</p>
            <p style="margin:0;font-family:Arial,Helvetica,sans-serif;font-size:13px;color:#bdc3ff;font-weight:700;">&#9733;&#9733;&#9733;&#9733;&#9733; &nbsp;${author}</p>
          </td>
        </tr>
      </table>\n`;
}
...
// ============================================================
// ═══════════════════════════════════════════════════════════
//  SEGMENT D — COMEBACK BLUEPRINT FUNNEL
//  Free Blueprint → Education Sequence → $17 Purchase
//
//  Email 1 (Day 0): Blueprint delivery — sent immediately on registration
//  Email 2 (Day 2): The Cost of Staying Stuck
//  Email 3 (Day 4): Why Motivation Isn't Enough
//  Email 4 (Day 6): The Comeback Framework
//  Email 5 (Day 8): What 90 Days Actually Looks Like
//  Email 6 (Day 10): The One Thing Stopping You
//  Email 7 (Day 12): Let's Talk About the $17
//  Email 8 (Day 14): This Is the Moment (Direct $17 Offer — No Urgency)
// ═══════════════════════════════════════════════════════════
// ============================================================

// ── SEGMENT D · EMAIL 1 — Blueprint Delivery ─────────────────
// Subject: Your Blueprint Is Here
// Send: Immediately on registration (Day 0)
// ⚠️  BLUEPRINT_URL env var overrides the default local route.
//     Default: /api/download/blueprint (serves blueprint.pdf from project root)
//     Upload blueprint.pdf to the project root to activate the download.
function blueprintEmail1(firstName, email) {
    const blueprintUrl = process.env.BLUEPRINT_URL
        || `${SITE_URL}/api/download/blueprint?email=${encodeURIComponent(email)}`;

    const body =
        ln(`Hey ${firstName},`) +
        ln(`Welcome. Your Blueprint is ready.`) +
        ln(`<strong>The Comeback: Unrecognizable Blueprint</strong> is a practical framework for rebuilding focus, discipline, productivity, and execution — starting where you actually are.`) +
        ln(`It explains the problem clearly, walks you through the framework, and gives you a map of the major phases. It is the roadmap. Not fluff.`) +
        ctaButton(blueprintUrl, 'ACCESS YOUR BLUEPRINT →') +
        ln(`Over the next two weeks, I will send you a short email every couple of days walking you through how to apply the framework and what the system behind it actually looks like.`) +
        ln(`No noise. No daily spam. Just the thinking that will make the Blueprint actually land.`) +
        ln(`If the Blueprint does not show up in your primary inbox, check your Promotions or Spam folder. Mark this address as safe so you do not miss the follow-up emails.`) +
        signOff();

    return {
        subject: `Your Blueprint Is Here`,
        html: plainLayout({ body, email, kicker: 'Blueprint Delivery' })
    };
}

// ── SEGMENT D · EMAIL 2 — The Cost of Staying Stuck ──────────
// Subject: The Cost of Staying Stuck
// Send: Day 2
function blueprintEmail2(firstName, email) {
    const body =
        ln(`Hey ${firstName},`) +
        ln(`There is a specific kind of person I keep seeing.`) +
        ln(`They are not lazy. They have goals. They have ideas. They start things. They work late sometimes. But they keep waking up six months later in the same place — more informed, still stuck.`) +
        ln(`The cost of that cycle is not just frustration. It compounds.`) +
        ln(`Every month you spend moving in circles is a month of output you do not get back. Not in income, not in skill, not in the kind of work that actually builds something. The starting-over loop looks like effort, but it costs you the one thing you do not get a refund on — time.`) +
        bentoBox('What staying stuck actually costs', [
            '🔁 Repeated starting-over drains motivation faster than failure does',
            '📉 Six months of partial effort beats zero — but still leaves you far behind where consistent work would have put you',
            '🧠 Your self-image adapts to the pattern — not the intention',
        ]) +
        ln(`The Blueprint you downloaded maps this out in more detail. If you have not gone through it yet — go back to that email and open it. The framework starts making sense once you see the full pattern.`) +
        ln(`Next email: why motivation works for getting started — and what you actually need to keep going.`) +
        signOff();

    return {
        subject: `The Cost of Staying Stuck`,
        html: plainLayout({ body, email, kicker: 'Blueprint Series · Day 2' })
    };
}

// ── SEGMENT D · EMAIL 3 — Why Motivation Isn't Enough ────────
// Subject: Why Motivation Isn't Enough
// Send: Day 4
function blueprintEmail3(firstName, email) {
    const body =
        ln(`Hey ${firstName},`) +
        ln(`Motivation is real. I am not going to tell you it does not exist.`) +
        ln(`It gets you started. It shows up when the idea is fresh, when you are fired up after a conversation, when you have just watched something that sparked you. That energy is real and it is worth using.`) +
        ln(`The problem is that motivation is <em>weather</em>. It comes and goes. And if the only thing holding your work ethic together is how you feel on a given morning, you are going to have a lot of missed days.`) +
        ln(`A repeatable system is different. It gives you something to fall back on when motivation disappears — because it will. What makes the difference between someone who builds something over 90 days and someone who does not is not raw drive. It is the presence of structure that makes the next step obvious even on a flat day.`) +
        bentoBox('Motivation vs. System', [
            '⚡ Motivation: high at the start, unpredictable after week two',
            '🔧 System: starts slower, compounds over time, survives low-energy days',
            '🏗️ The goal is to make execution a default — not a decision you make every morning',
        ]) +
        ln(`The Blueprint outlines the principles. What comes next — in the actual Comeback system — is the daily structure that removes the "what do I do today" question completely.`) +
        ln(`Next email: the actual framework and the three phases of the Comeback system.`) +
        signOff();

    return {
        subject: `Why Motivation Isn't Enough`,
        html: plainLayout({ body, email, kicker: 'Blueprint Series · Day 4' })
    };
}

// ── SEGMENT D · EMAIL 4 — The Comeback Framework ─────────────
// Subject: The Comeback Framework
// Send: Day 6
function blueprintEmail4(firstName, email) {
    const body =
        ln(`Hey ${firstName},`) +
        ln(`The Blueprint lays this out in full, but here is the core of the Comeback framework:`) +
        ln(`<strong>The problem is not that you are undisciplined. The problem is that you do not have a clear operating system for your day.</strong>`) +
        ln(`Most people try to get more done by adding more — more routines, more tools, more accountability — on top of a foundation that does not hold. The Comeback framework works the other way. It starts by removing the friction and confusion that makes consistent execution impossible.`) +
        bentoBox('The three phases of the Comeback system', [
            '📍 Phase 1 — Establish: Build the foundation. Daily Scoreboard, Focus Firewall, and a personal operating system that removes daily decision fatigue.',
            '🔁 Phase 2 — Execute: Run the system. Show up to your defined work sessions, track output (not just hours), and build the proof-of-consistency that rebuilds your self-image.',
            '📈 Phase 3 — Build: Stack the wins. Once consistency is a habit, you layer on output — the actual projects, skills, and income that were the goal the whole time.',
        ]) +
        ln(`The Blueprint gives you the overview. But the map is not the territory. The Comeback: Unrecognizable system is the implementation layer — the daily tools, walkthroughs, and structure that makes the framework actually run.`) +
        ln(`Next: what 90 days of this actually looks like in practice.`) +
        signOff();

    return {
        subject: `The Comeback Framework`,
        html: plainLayout({ body, email, kicker: 'Blueprint Series · Day 6' })
    };
}

// ── SEGMENT D · EMAIL 5 — What 90 Days Actually Looks Like ───
// Subject: What 90 Days Actually Looks Like
// Send: Day 8
function blueprintEmail5(firstName, email) {
    const body =
        ln(`Hey ${firstName},`) +
        ln(`Ninety days is long enough to change the way you work. It is short enough that you can see the end from the start.`) +
        ln(`Here is what the structure actually looks like, week by week:`) +
        bentoBox('90-Day Comeback — What to expect', [
            '<strong>Weeks 1–2:</strong> Set up your Daily Scoreboard, Focus Firewall, and personal work protocol. Eliminate the daily "what do I do today" problem.',
            '<strong>Weeks 3–6:</strong> Run the system. Build consistency. Track output. This is where most people start to notice the difference between deciding to work and just working.',
            '<strong>Weeks 7–10:</strong> Output starts compounding. The projects you started in week 1 have traction. You are no longer "getting ready to build" — you are building.',
            '<strong>Weeks 11–13:</strong> Consolidate and lock it in. By now the system has become your default, not a discipline experiment.',
        ]) +
        ln(`The 90-day structure is not a rigid schedule. It is a container with clear checkpoints. The daily work adapts to what you are building — the system stays consistent.`) +
        ln(`Inside Comeback: Unrecognizable, this structure is the backbone. Along with the Daily Scoreboard, Focus Firewall, challenge protocols, community, and direct access to the Ask Adams AI — it is built to run this exact arc.`) +
        ln(`Two more emails this week — then I will tell you what the full system costs.`) +
        signOff();

    return {
        subject: `What 90 Days Actually Looks Like`,
        html: plainLayout({ body, email, kicker: 'Blueprint Series · Day 8' })
    };
}

// ── SEGMENT D · EMAIL 6 — The One Thing Stopping You ─────────
// Subject: The One Thing Stopping You
// Send: Day 10
function blueprintEmail6(firstName, email) {
    const body =
        ln(`Hey ${firstName},`) +
        ln(`Most people think what is stopping them is time, money, circumstances, the right skill, the right opportunity.`) +
        ln(`Sometimes it is those things. But the more common barrier I see is simpler than any of those.`) +
        ln(`<strong>It is the execution gap.</strong>`) +
        ln(`The execution gap is the distance between what you know you should do and what you actually do consistently. Most people are not short on information. They know what to work on. They know discipline matters. They have heard the ideas. The gap is in doing it repeatedly — in the face of distraction and low motivation — without a structure that makes the next step automatic.`) +
        ln(`This is what the Comeback system is built to close. Not through motivation or willpower, but through design — removing friction so that showing up is the path of least resistance rather than the path of most effort.`) +
        bentoBox('Closing the execution gap', [
            '🎯 Clarity: Know exactly what you are working on and why — every day',
            '🔒 Focus Firewall: A defined protocol for protecting your deep work time',
            '📊 Daily Scoreboard: Track what you actually did, not just what you planned',
            '👥 Accountability: Community + Ask Adams AI for when you hit friction',
        ]) +
        ln(`Next email: a direct conversation about the $17.`) +
        signOff();

    return {
        subject: `The One Thing Stopping You`,
        html: plainLayout({ body, email, kicker: 'Blueprint Series · Day 10' })
    };
}

// ── SEGMENT D · EMAIL 7 — Let's Talk About the $17 ───────────
// Subject: Let's Talk About the $17
// Send: Day 12
function blueprintEmail7(firstName, email) {
    const shopUrl = whopLink();
    const body =
        ln(`Hey ${firstName},`) +
        ln(`I want to be direct with you about something.`) +
        ln(`Comeback: Unrecognizable is $17. Not $97. Not $297. Not $17 for now and then $68. Just $17.`) +
        ln(`I priced it this way because I wanted price to not be the reason someone does not try this. If someone goes through the system and it does not change how they work, I do not want them to feel like they lost much. And if it does work — $17 will be the best purchase decision they made this year.`) +
        ln(`What you get at $17:`) +
        bentoBox("Comeback: Unrecognizable — What's included", [
            '📅 The 90-day Comeback structure with clear phase checkpoints',
            '📊 Daily Scoreboard — your daily execution tracking system',
            '🔒 Focus Firewall — your deep work protection protocol',
            '🧩 Challenge protocols for rebuilding discipline and output',
            '👥 Community access — accountability and peer execution',
            '🤖 Ask Adams AI — your system-aware support tool',
            '🎓 Video walkthroughs and implementation guidance',
        ]) +
        ln(`The Blueprint you downloaded gives you the map. The system gives you the vehicle.`) +
        ln(`<em>"The Blueprint shows you what to change. Comeback gives you the system to actually do it."</em>`) +
        ln(`Tomorrow: the final email in this series.`) +
        signOff();

    return {
        subject: `Let's Talk About the $17`,
        html: plainLayout({ body, email, kicker: 'Blueprint Series · Day 12' })
    };
}

// ── SEGMENT D · EMAIL 8 — Direct Offer ($17 — No Urgency) ────
// Subject: This Is the Moment
// Send: Day 14
// Permanent $17 price. No countdown. No deadline. No fake scarcity.
function blueprintEmail8(firstName, email) {
    const shopUrl = whopLink();
    const body =
        ln(`Hey ${firstName},`) +
        ln(`This is the last email in the Blueprint series.`) +
        ln(`Over the past two weeks, I have walked you through the framework, the phases, what 90 days actually looks like, and what is inside the Comeback system. You have seen the map. The question now is whether you want the system.`) +
        ln(`Comeback: Unrecognizable is $17. The price is not going to change. There is no deadline. No countdown. No manufactured urgency.`) +
        ln(`If this is the right time for you — the link is below.`) +
        ctaButton(shopUrl, 'GET COMEBACK: UNRECOGNIZABLE — $17') +
        ln(`If now is not the right time, that is completely fine. Keep the Blueprint. Come back when you are ready. The system is not going anywhere.`) +
        ln(`And if you have a question before deciding — just reply to this email. I read every one.`) +
        signOff();

    return {
        subject: `This Is the Moment`,
        html: plainLayout({ body, email, kicker: 'Blueprint Series · Day 14' })
    };
}

// ── SEGMENT C · CORRECTION EMAIL ─────────────────────────────
// Subject: Update: Comeback: Unrecognizable is staying at $17
// Send: One-time to all active Segment C subscribers
// Replaces the old urgency/price-increase messaging with the truth.
function segCCorrectionEmail(firstName, email) {
    const shopUrl = whopLink();
    const body =
        ln(`Hey ${firstName},`) +
        ln(`I want to send you a quick update before anything else lands in your inbox.`) +
        ln(`If you received recent emails from me about a price increase — $17 moving to $68 — that messaging is no longer applicable. The $17 price is not changing.`) +
        ln(`I have made the decision to keep Comeback: Unrecognizable at $17 permanently. No deadline. No countdown. No future price increase planned.`) +
        ln(`If you have been on the fence, the price is not the reason to hesitate. $17 is the real price. That is not a sale — it is just what it costs.`) +
        ctaButton(shopUrl, 'GET COMEBACK: UNRECOGNIZABLE — $17') +
        ln(`If you are not interested, no hard feelings — you can unsubscribe below and I will not email you again.`) +
        ln(`And if you have questions before deciding, reply to this email. I read everything.`) +
        signOff();

    return {
        subject: `Update: Comeback: Unrecognizable is staying at $17`,
        html: plainLayout({ body, email, kicker: 'Price Update' })
    };
}

// ── OLD-LEAD REACTIVATION EMAIL ───────────────────────────────
// Subject: I changed the way I'm doing this
// Send: One-time to ~68 old waitlist leads (active, pre-Blueprint)
// Invites them to re-enter through the new free Blueprint funnel.
function oldLeadReactivationEmail(firstName, email) {
    const blueprintUrl = `${SITE_URL}/blueprint`;
    const body =
        ln(`Hey ${firstName},`) +
        ln(`If you have been on my list for a while, you may have noticed I went quiet.`) +
        ln(`Here is what happened: I was running a launch with urgency emails and countdown timers, and I stopped. That approach was not right for what I am building, and it was not honest.`) +
        ln(`I am not going to pretend $17 has an expiry date. It does not. And I am not going to tell you the door is closing when it is not.`) +
        ln(`What I am doing instead:`) +
        ln(`I built a free resource — the <strong>Comeback: Unrecognizable Blueprint</strong>. It is a practical framework for rebuilding focus, discipline, and execution. No opt-in bait. No fake value. Just the actual map behind the system.`) +
        ln(`If you are still interested in what I am building, you can grab the Blueprint for free. It will also put you on a short email series that explains the full system properly — no pressure, no countdown.`) +
        ctaButton(blueprintUrl, 'GET THE FREE BLUEPRINT →') +
        ln(`If you are not interested anymore, I get it — use the unsubscribe link below and I will remove you completely.`) +
        ln(`Either way, thanks for being on the list. I would rather be honest with you about the change than pretend nothing happened.`) +
        signOff();

    return {
        subject: `I changed the way I'm doing this`,
        html: plainLayout({ body, email, kicker: 'A Note from Adams' })
    };
}
...
function segAEmail1(firstName, email) {
    ...
    const body =
        ...
        ctaButton(shopUrl, 'Get Access Now — $17') +
...
function segAEmail2(firstName, email) {
    ...
    const body =
        ...
        ctaButton(shopUrl, 'Join the Protocol — $17') +
        signOff();
...
function segAEmail3(firstName, email) {
    ...
    const body =
        ...
        ctaButton(shopUrl, 'Get Access — $17') +
        signOff() +
...
function segAEmail5(firstName, email) {
    ...
    const body =
        ...
        ctaButton(shopUrl, 'Start the 90 Days — $17') +
        signOff();
...
function segAEmail6(firstName, email) {
    ...
    const body =
        ...
        ctaButton(shopUrl, 'Join the Community — $17') +
        signOff();
...
function segAEmail7(firstName, email) {
    ...
    const body =
        ...
        ctaButton(shopUrl, 'Claim the $17 Price Now') +
...
function segAEmail8(firstName, email) {
    ...
    const body =
        ...
        ctaButton(shopUrl, 'Get Instant Access — $17 →') +
...
function segAEmail9(firstName, email) {
    ...
    const body =
        ...
        ctaButton(shopUrl, 'Get Access — $17 (3 Days Left)') +
...
function segAEmail10(firstName, email) {
    ...
    const body =
        ...
        ctaButton(shopUrl, 'Lock In $17 Before Midnight →') +
        guaranteeBadge +
...
function segAEmail11(firstName, email) {
    ...
    const body =
        ...
        ctaButton(shopUrl, 'Join Now Before Midnight — $17') +
        guaranteeBadge +
...
function segBEmail1(firstName, email) {
    ...
    const body =
        ...
        ctaButton(shopUrl, 'Get the Full System — $17') +
...
function postPurchaseEmail1(firstName, email) {
    const shopUrl = 'https://whop.com/hub';
    const body =
        ln(`Hey ${firstName},`) +
        ln(`Welcome to <strong>Comeback: Unrecognizable</strong>.`) +
        ...
        bentoBox('Your First Moves Inside Whop', [
            '<strong>1. The Unrecognizable Blueprint PDF</strong> — read it cover to cover before doing anything else.',
            '<strong>2. Your 12 Modules</strong> — open Module 1 today and review Phase 1.',
            '<strong>3. The Community</strong> — introduce yourself in the group (first name, location, goal).',
            '<strong>4. Daily Scoreboard</strong> — open the Interactive Web Scoreboard and lock in Day 1.'
        ]) +
        ctaButton(shopUrl, 'Open Your Whop Dashboard →', true) +
        ln(`Don't try to do all of this tonight. Do one thing: read the blueprint. Then come back tomorrow.`) +
    ...
    return {
        subject: `you're in. here's your first move.`,
        html: plainLayout({ body, email, kicker: 'VIP Access' })
    };
}
...
function postPurchaseEmail2(firstName, email) {
    ...
    return {
        subject: `your only job today`,
        html: plainLayout({ body, email, kicker: 'Day 2 Focus' })
    };
}
...
function postPurchaseEmail3(firstName, email) {
    ...
    return {
        subject: `one week in — real talk`,
        html: plainLayout({ body, email, kicker: 'Day 7 Checkpoint' })
    };
}
...
function postPurchaseEmailReview(firstName, email) {
    ...
    return {
        subject: `quick favor — how's Comeback: Unrecognizable going for you?`,
        html: plainLayout({ body, email, kicker: 'Review Request' })
    };
}

// ── CUSTOMER RE-ACTIVATION & CHECK-IN (FOR QUIET/INACTIVE BUYERS) ──
// Subject: Checking in on your Comeback progress (quick personal note)
// Send: Re-engage paying members who haven't logged in recently
function customerReactivationEmail(firstName, email) {
    const dashboardUrl = 'https://whop.com/hub';
    const body =
        ln(`Hey ${firstName},`) +
        ln(`I wanted to send you a quick personal note to check in.`) +
        ln(`You joined <strong>Comeback: Unrecognizable</strong> a little while ago, and I noticed you haven't been as active in the community or dashboard recently.`) +
        ln(`Life gets busy — and if you got pulled into work or life happened, ${gold(`there is zero guilt.`)} You didn't fail, and you didn't fall behind. The beauty of this system is that it's waiting for you the exact second you're ready to lock in.`) +
        bentoBox('Everything In Your Member Hub Ready Right Now:', [
            '<strong>12-Module Day-by-Day Plan</strong> — Phase 1 is built for a 15-minute quick restart.',
            '<strong>The Focus Scoreboard App</strong> — log your deep work blocks & track momentum.',
            '<strong>Private Member Community</strong> — check in with other members running the reps.',
            '<strong>24/7 AI Mentor Clone</strong> — available anytime for instant guidance.'
        ]) +
        ctaButton(dashboardUrl, 'Open Your Member Dashboard →', true) +
        ln(`<strong>Your next move today:</strong> Just log in, open Module 1, and read the first 3 pages. Don't worry about 90 days right now — just win today.`) +
        ln(`If you're stuck on anything or need help getting momentum back, just hit reply. I'm right here.`) +
        signOff();

    return {
        subject: `Checking in on your Comeback progress (quick personal note)`,
        html: plainLayout({ body, email, kicker: 'Member Check-In' })
    };
}
...
function incompletePaymentEmail(firstName, email) {
    ...
    const body =
        ...
        bentoBox('What’s Unlocked The Second You Complete:', [
            '<strong>12-Module Day-by-Day Rebuild Plan</strong>',
            '<strong>Focus Firewall Protocol & Daily Scoreboard App</strong>',
            '<strong>Private Whop Community & 24/7 AI Mentor Clone</strong>',
            '<strong>Dedicated Weekly Q&A Support & 10X Guarantee</strong>'
        ]) +
        ctaButton(shopUrl, 'Complete Your Access — $17 →', true) +
    ...
    return {
        subject: `quick question about your Comeback access`,
        html: plainLayout({ body, email, kicker: 'Recovery Link' })
    };
}
...
//  Upgraded to Midnight Executive system layout & components
// ═══════════════════════════════════════════════════════════
// ============================================================
...
function emailLayout({ dayLabel, headline, bodyHtml, footerExtra, email }) {
    const headlineBlock = headline
        ? sectionTitle(headline, '#ffffff')
        : '';

    const body = `
        ${headlineBlock}
        ${bodyHtml}
        ${footerExtra ? `<div style="margin-top:20px;">${footerExtra}</div>` : ''}
    `;

    return plainLayout({
        body,
        email,
        kicker: dayLabel || 'Adams X Protocol',
        title: headline ? `${headline} — Adams X` : 'Adams X Project'
    });
}

function p(text) {
    return `<p style="font-family:Arial,Helvetica,sans-serif;font-size:15px;line-height:1.72;color:#CBD5E1;margin:0 0 16px;letter-spacing:-0.01em;">${text}</p>`;
}

function pMuted(text) {
    return `<p style="font-family:Arial,Helvetica,sans-serif;font-size:14px;line-height:1.7;color:#94A3B8;margin:0 0 18px;font-style:italic;">${text}</p>`;
}

function legacySignOff() {
    return `
        <div style="margin-top:28px;padding-top:20px;border-top:1px solid rgba(255,255,255,0.08);">
            <p style="font-family:Arial,Helvetica,sans-serif;font-size:15px;font-weight:700;color:#ffffff;margin:0 0 4px;">&mdash; Adams</p>
            <p style="font-family:Arial,Helvetica,sans-serif;font-size:12px;color:#94A3B8;margin:0;">Adams X Project</p>
        </div>`;
}
...
function legacySignOffFull() {
    return `
        <div style="margin-top:28px;padding-top:20px;border-top:1px solid rgba(255,255,255,0.08);">
            <p style="font-family:Arial,Helvetica,sans-serif;font-size:15px;font-weight:700;color:#ffffff;margin:0 0 4px;">&mdash; Adams</p>
            <p style="font-family:Arial,Helvetica,sans-serif;font-size:12px;color:#94A3B8;margin:0 0 10px;">Adams X Project</p>
            <p style="font-family:Georgia,serif;font-size:13.5px;color:#CBD5E1;margin:0;font-style:italic;">I am watching from afar and I am rooting for you.</p>
        </div>`;
}

function actionBox(title, text) {
    return `
        <table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0" style="background:#0d122b;border:1px solid rgba(82,97,179,0.35);border-left:4px solid #5261b3;border-radius:10px;margin:20px 0;overflow:hidden;">
          <tr>
            <td style="padding:18px 22px;">
              <p style="margin:0 0 8px;font-size:12px;font-weight:800;letter-spacing:0.14em;color:#bdc3ff;text-transform:uppercase;font-family:Arial,Helvetica,sans-serif;">${title}</p>
              <p style="margin:0;font-size:14px;line-height:1.65;color:#E2E8F0;font-family:Arial,Helvetica,sans-serif;">${text}</p>
            </td>
          </tr>
        </table>`;
}
...
function divider() {
    return `<div style="height:1px;background:rgba(255,255,255,0.08);margin:24px 0;"></div>`;
}
    ...
    return {
        ...
        html: emailLayout({
            ...
            bodyHtml: `
                ...
                <table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0" style="background:#0d122b;border:1px solid rgba(82,97,179,0.35);border-radius:10px;margin:28px 0;overflow:hidden;">
                  <tr><td style="padding:36px 28px;text-align:center;">
                    <p style="font-family:Arial,Helvetica,sans-serif;font-size:22px;color:#ffffff;margin:0 0 8px;font-weight:700;letter-spacing:-0.04em;">Comeback: Unrecognizable</p>
                    <p style="font-family:Arial,Helvetica,sans-serif;font-size:12px;color:#bdc3ff;text-transform:uppercase;letter-spacing:0.15em;margin:0 0 20px;">7 Rules to Change Your Life</p>
                  </td></tr>
                </table>
                ${ctaButton(WAITLIST_URL, 'Get Access Now — $17')}
                ${legacySignOffFull()}
            `
        })
    };
    ...
    return {
        ...
        html: emailLayout({
            ...
            bodyHtml: `
                ...
                ${p(`The full blueprint is still waiting for you. <strong>Comeback: Unrecognizable</strong> — the complete 90-day protocol. Only <span style="text-decoration:line-through;color:#64748B;">$67</span> &rarr; <span style="color:#bdc3ff;font-weight:700;font-size:16px;">$17 Launch Offer</span>.`)}
...
function day14(firstName, email) {
    return {
        ...
        html: emailLayout({
            ...
            bodyHtml: `
                ${p(`${firstName}, <strong>Comeback: Unrecognizable</strong> gives you an operating system. 90 days. 7 rules. <span style="text-decoration:line-through;color:#64748B;">$67</span> &rarr; <span style="color:#bdc3ff;font-weight:700;font-size:16px;">$17 Launch Offer</span>.`)}
                ${ctaButton('Get the Full Blueprint — $17 Launch Offer', WAITLIST_URL)}
                ${legacySignOff()}
            `
        })
    ...
    return {
        ...
        html: emailLayout({
            ...
            bodyHtml: `
                ...
                ${p(`<strong>Comeback: Unrecognizable — <span style="text-decoration:line-through;color:#64748B;font-weight:400;">$67</span> &rarr; <span style="color:#bdc3ff;">$17 Launch Offer</span>.</strong>`)}
...
function launchBlastEmail(firstName, email) {
    const shopUrl = whopLink();
    const heroBanner = `<div style="margin:0 0 24px;"><img src="https://adamsxproject.com.ng/images/comeback-hero.png" alt="Comeback: Unrecognizable" width="100%" style="display:block;max-width:100%;border-radius:4px;" /></div>`;
    // Using module-level accent() and testimonialBox() helpers
    const body =
        heroBanner +
        ln(`Hey ${firstName},`) +
        ...
        ln(`${accent(`$17 is not the question.`)} The question is: what is staying exactly where you are costing you ${accent(`every single day?`)}`) +
        ln(`This window is open for 6 days. After that, $67 permanently. No extensions, no exceptions.`) +
        // CTA bottom
        ctaButton(shopUrl, 'Claim Your Spot — $17 →') +
...
// ============================================================
// ═══════════════════════════════════════════════════════════
//  SEGMENT C — 7-EMAIL PRICE INCREASE URGENCY SEQUENCE ($17 → $68)
// ═══════════════════════════════════════════════════════════
// ============================================================

// ── SEGMENT C · EMAIL 1 (Day 0) ──────────────────────────────
// Subject: You're one step from Comeback: Unrecognizable
function segCEmail1(firstName, email) {
    const shopUrl = whopLink();
    const body =
        ln(`Hey ${firstName},`) +
        ln(`You showed interest in <strong>Comeback: Unrecognizable</strong>. You're not locked in yet — but you're closer than you think.`) +
        ln(`Here's exactly what happens once you get inside:`) +
        bentoBox('What’s Inside The System', [
            '<strong>The Unrecognizable Blueprint</strong> — a full 90-day, day-by-day plan across 4 phases (The Leak, The Lock, The Reps, The Shift). No guessing what to do each morning. You open it, you execute.',
            '<strong>The Impossible-to-Distract Firewall</strong> — the exact protocol that kills the distractions responsible for every other system you\'ve tried and dropped.',
            '<strong>Your Daily Scoreboard</strong> — real numbers, not vibes. You watch your output and income compound, day by day.'
        ]) +
        ln(`That's just the start. Full breakdown coming over the next few days.`) +
        ln(`Right now, it's <strong>$17</strong>.`) +
        ctaButton(shopUrl, 'Get Instant Access — $17 →', true) +
        signOff('— Adams');

    return {
        subject: `You're one step from Comeback: Unrecognizable`,
        html: plainLayout({ body, email, kicker: 'Launch Urgency · Step 1' })
    };
}

// ── SEGMENT C · EMAIL 2 (Day 1) ──────────────────────────────
// Subject: Why I built this (2 min read)
function segCEmail2(firstName, email) {
    const shopUrl = whopLink();
    const videoThumbnail = `      <table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0" style="margin:20px 0;">
        <tr>
          <td align="center">
            <a href="${shopUrl}" target="_blank" style="text-decoration:none;display:block;">
              <table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0" style="max-width:480px;background:#050505;border-radius:14px;border:1px solid rgba(0,210,255,0.3);overflow:hidden;box-shadow:0 12px 30px rgba(0,0,0,0.85);">
                <tr>
                  <td align="center" style="padding:48px 20px;background:radial-gradient(circle at 50% 50%, rgba(0, 136, 255, 0.25) 0%, rgba(3, 8, 20, 0.95) 100%);">
                    <div style="display:inline-block;background:linear-gradient(135deg, #00D2FF 0%, #0088FF 100%);width:52px;height:52px;border-radius:50%;line-height:52px;text-align:center;color:#000;font-size:22px;box-shadow:0 0 24px rgba(0,210,255,0.7);margin-bottom:12px;">▶</div>
                    <div style="font-size:12px;font-weight:800;color:#FFFFFF;letter-spacing:0.12em;text-transform:uppercase;">Watch Walkthrough Video (2 Min)</div>
                  </td>
                </tr>
              </table>
            </a>
    ...
    const body =
        ln(`Hey ${firstName},`) +
        videoThumbnail +
        ln(`<strong>Video script for recording:</strong>`) +
        ln(`[00:00] <em>"I'm going to be honest with you for a second. Most productivity systems are built by people who've never actually struggled with focus. They give you a checklist and disappear. I built Comeback: Unrecognizable because I needed the opposite — a system that assumes you've already failed at this before, and builds around that instead of pretending it won't happen again."</em>`) +
        ln(`[00:20] <em>"Inside, you get four phases. The Leak — where you plug the exact places your time and focus are bleeding out right now. The Lock — where you commit to one thing and cut the noise. The Reps — where you build the habit through actual repetition, not motivation. And The Shift — where you prove to yourself, with real numbers, that you're not the same person who started."</em>`) +
        ln(`[00:50] <em>"It's $17 right now. That's not a permanent price. I wanted you to hear this from me directly first: this isn't hype. It's a system. Go see it for yourself."</em>`) +
        ctaButton(shopUrl, 'Watch the full walkthrough & get access →', true) +
        signOff('— Adams');

    return {
        subject: `Why I built this (2 min read)`,
        html: plainLayout({ body, email, kicker: 'Video Walkthrough' })
    };
}

// ── SEGMENT C · EMAIL 3 (Day 2) ──────────────────────────────
// Subject: What's actually inside (part 1)
function segCEmail3(firstName, email) {
    const shopUrl = whopLink();
    const body =
        ln(`Hey ${firstName},`) +
        bentoBox('System Breakdown (Part 1)', [
            '<strong>[Feature 1] THE UNRECOGNIZABLE BLUEPRINT</strong><br><span style="color:#CBD5E1;">What it is:</span> your full 90-day roadmap, broken into 4 phases and 12 weekly challenges, each with a hard pass/fail condition — no vague self-rating.<br><span style="color:#CBD5E1;">What it\'s for:</span> removing every ounce of "what do I do today" so you just execute.',
            '<strong>[Feature 2] THE IMPOSSIBLE-TO-DISTRACT FIREWALL</strong><br><span style="color:#CBD5E1;">What it is:</span> a complete digital perimeter audit — app blocks, phone locks, notification kill-switches, plus energy management.<br><span style="color:#CBD5E1;">What it\'s for:</span> protecting the 4-hour deep work blocks where your actual output happens.',
            '<strong>[Feature 3] YOUR DAILY SCOREBOARD</strong><br><span style="color:#CBD5E1;">What it is:</span> a live tracker for your streaks, hours, output, and revenue.<br><span style="color:#CBD5E1;">What it\'s for:</span> you see your consistency compound in real numbers instead of hoping it\'s working.'
        ]) +
        ln(`Still $17. Going up soon — I'll tell you exactly when in a few days.`) +
        ctaButton(shopUrl, 'Get Access Now — $17 →', true) +
        signOff('— Adams');

    return {
        subject: `What's actually inside (part 1)`,
        html: plainLayout({ body, email, kicker: 'Part 1 Architecture' })
    };
}

// ── SEGMENT C · EMAIL 4 (Day 3) ──────────────────────────────
// Subject: What's actually inside (part 2)
function segCEmail4(firstName, email) {
    const shopUrl = whopLink();
    const body =
        ln(`Hey ${firstName},`) +
        bentoBox('System Breakdown (Part 2)', [
            '<strong>[Feature 4] WATCH ME BUILD IT (Module Walkthroughs)</strong><br><span style="color:#CBD5E1;">What it is:</span> 12 over-the-shoulder video breakdowns, one per weekly challenge, showing exactly how each one is done.<br><span style="color:#CBD5E1;">What it\'s for:</span> you\'re not guessing from a PDF — you\'re watching it happen, then doing it yourself.',
            '<strong>[Feature 5] 12 WEEKLY MOMENTUM CHALLENGES</strong><br><span style="color:#CBD5E1;">What it is:</span> a structured checkpoint every single week — Information Blackout, Skill Lockdown, Ship the Rough Version, Name the Wall, and more.<br><span style="color:#CBD5E1;">What it\'s for:</span> turning a 90-day marathon into 12 manageable sprints.',
            '<strong>[Feature 6] THE GROUP THAT WON\'T LET YOU QUIT</strong><br><span style="color:#CBD5E1;">What it is:</span> a private accountability community of creators and builders running the same protocol.<br><span style="color:#CBD5E1;">What it\'s for:</span> solo discipline fails on bad days. This is the backup.'
        ]) +
        ctaButton(shopUrl, 'Get Access Now — $17 →', true) +
        signOff('— Adams');
    ...
    return {
        subject: `What's actually inside (part 2)`,
        html: plainLayout({ body, email, kicker: 'Part 2 Community' })
    };
}

// ── SEGMENT C · EMAIL 5 (Day 4) ──────────────────────────────
// Subject: The part most people don't expect
function segCEmail5(firstName, email, daysLeft = 3) {
    const shopUrl = whopLink();
    const dynamicDays = typeof daysLeft === 'number' ? daysLeft : 3;
    const body =
        ln(`Hey ${firstName},`) +
        bentoBox('The Complete Arsenal', [
            '<strong>[Feature 7] ASK ADAMS AI (24/7 ACCESS)</strong><br><span style="color:#CBD5E1;">What it is:</span> my entire discipline and coaching approach, trained into an AI you can message anytime.<br><span style="color:#CBD5E1;">What it\'s for:</span> the moment your focus breaks at 2am, you\'ve still got backup.',
            '<strong>[Feature 8] DEDICATED SUPPORT</strong><br><span style="color:#CBD5E1;">What it is:</span> weekly Q&A calls, a personal welcome call the moment you join, and a success team.'
        ]) +
        testimonialBox('I’ve been in the program for 3 weeks and my focus hours went from 1.5h to 5h a day. The scoreboard doesn’t let me lie to myself.', 'Marcus K., Verified Member') +
        ln(`You've now seen everything. $17 gets you all 8 of these. That price moves to <strong>$68</strong> in <strong>${dynamicDays} days</strong>.`) +
        ctaButton(shopUrl, 'Lock In $17 Before It Increases →', true) +
        signOff('— Adams');

    return {
        subject: `The part most people don't expect`,
        html: plainLayout({ body, email, kicker: 'Price Alert' })
    };
}

// ── SEGMENT C · EMAIL 6 (Day 5) ──────────────────────────────
// Subject: Price goes to $68 in 48 hours
function segCEmail6(firstName, email) {
    const shopUrl = whopLink();
    const body =
        ln(`Hey ${firstName},`) +
        ln(`Straight talk: this price won't stay at $17.`) +
        ln(`In <strong>48 hours</strong>, <strong>Comeback: Unrecognizable moves to $68</strong>. Same 8 features, same system, same access — just a different number.`) +
        ln(`If you've been meaning to get in, this is the point where waiting starts costing you money instead of saving it.`) +
        ctaButton(shopUrl, 'Get In at $17 — Before It\'s Gone →', true) +
        signOff('— Adams');

    return {
        subject: `Price goes to $68 in 48 hours`,
        html: plainLayout({ body, email, kicker: '48 Hours Left' })
    };
}

// ── SEGMENT C · EMAIL 7 (Day 6/7) ────────────────────────────
// Subject: Closing tonight — $17 ends
function segCEmail7(firstName, email) {
    const shopUrl = whopLink();
    const body =
        ln(`Hey ${firstName},`) +
        ln(`Last email on this.`) +
        ln(`<strong>Tonight, the $17 price closes. Tomorrow it's $68.</strong>`) +
        ln(`You've seen what's inside — the Blueprint, the Firewall, the Scoreboard, the walkthroughs, the community, the AI, the support. Nothing changes about what you get. Only the price does.`) +
        ln(`If you're in, this is the moment.`) +
        ctaButton(shopUrl, 'Get Access Before Midnight — $17 →', true) +
        ln(`Not for you right now? No hard feelings — reply and let me know why, I actually read these.`) +
        signOff('— Adams');

    return {
        subject: `Closing tonight — $17 ends`,
        html: plainLayout({ body, email, kicker: 'Final Notice' })
    };
}

module.exports = {
    // ── Segment D — Comeback Blueprint Funnel ──────────────────
    blueprintEmail1, blueprintEmail2, blueprintEmail3, blueprintEmail4,
    blueprintEmail5, blueprintEmail6, blueprintEmail7, blueprintEmail8,

    // ── Segment C Correction + Old-Lead Reactivation ──────────
    segCCorrectionEmail,
    oldLeadReactivationEmail,
    ...
    // ── Post-Purchase & Member Check-In ───────────────────────
    postPurchaseEmail1, postPurchaseEmail2, postPurchaseEmail3, postPurchaseEmailReview, customerReactivationEmail,

    // ── Segment C (Price-Increase Urgency — DEACTIVATED) ──────
    // Preserved for historical record only. No new enrollments.
    segCEmail1, segCEmail2, segCEmail3, segCEmail4,
    segCEmail5, segCEmail6, segCEmail7,
