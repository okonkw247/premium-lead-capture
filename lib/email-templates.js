...
const WAITLIST_URL = process.env.WHOP_PRODUCT_URL || 'https://whop.com/checkout/plan_vqAyJUoRV3oTP';
...
function launchBlastEmail(firstName, email) {
    ...
    const body =
        ln(`Hey ${firstName},`) +
        ln(`A while ago, you signed up for either the Monk Mode Starter Kit or the waitlist for my upcoming 90-day protocol: <strong>Comeback: Unrecognizable</strong>.`) +
        ln(`I'm writing to you today because the program is officially live.`) +
        ln(`But before I give you the link, I want to tell you about a DM I got recently.`) +
        ln(`A young creator messaged me: <em>"Adams, can you help me earn money? I need to help my parents."</em>`) +
        ln(`It's a message I get almost daily. And my answer is always the same: if you're struggling to make progress, you don't have a lack of information. You have an execution problem.`) +
        ln(`You download kits, watch YouTube videos on 'discipline', and plan out perfect routines. It feels like effort, but it asks nothing of you because nobody can watch you fail at watching a video.`) +
        ln(`The real shift happens when you have a system that forces you to execute day-by-day.`) +
        ln(`That's what <strong>Comeback: Unrecognizable</strong> is. It's a complete 90-day protocol:`) +
        ln(`— <strong>Your Day-by-Day Rebuild Plan</strong> (12 modules, 4 phases)<br>` +
           `— <strong>A Daily Scoreboard</strong> to track your focus hours and habits in cold, hard numbers<br>` +
           `— <strong>The Group That Won't Let You Quit</strong> (private community inside Whop of people running the same 90 days alongside you)`) +
        ln(`Here is what people who tested the early modules are saying:`) +
        ln(`<em>"I've been in the program for 3 weeks and my focus hours went from 1.5h to 5h a day. The scoreboard doesn't let me lie to myself."</em><br><strong>— Marcus K.</strong>`) +
        ln(`<em>"Adams' agency model in Phase 4 is so practical. Closed my first $1,500 retainer client on day 35."</em><br><strong>— Sarah M.</strong>`) +
        ln(`The public price on Whop is going to be $67. But because you are on this list, you can grab it today for the launch price of $17.`) +
        ln(`Here is the direct link to get instant access:`) +
        ln(link(shopUrl, shopUrl)) +
