...
// ====================================================// ============================================================
// MONETISE × APPLE EXECUTIVE EMAIL SYSTEM
// Sleek dark-mode obsidian aesthetic, SF Pro typography,
// cyan light flares, pill badges, and bento cards.
// Bulletproof table-based HTML compatible across all clients.
// ============================================================
function plainLayout({ body, email, kicker = 'Adams X Protocol' }) {
...
<body style="margin:0;padding:0;background-color:#030712;font-family:-apple-system,BlinkMacSystemFont,'SF Pro Display','Segoe UI',Roboto,Helvetica,Arial,sans-serif;-webkit-font-smoothing:antialiased;">
  
  <!-- Outer Wrapper Table -->
  <table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0" style="background-color:#030712;width:100%;table-layout:fixed;">
    <tr>
      <td align="center" style="padding:32px 14px 48px;">
        
        <!-- Main Email Glass Card (Max Width 580px) -->
        <table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0" style="max-width:580px;background:#081122;border:1px solid rgba(0, 210, 255, 0.22);border-radius:20px;overflow:hidden;box-shadow:0 25px 60px -10px rgba(0,0,0,0.95),0 0 45px rgba(0,136,255,0.12);">
          
          <!-- Top Cyan Glow Flare -->
          <tr>
            <td height="3" style="background:linear-gradient(90deg, transparent 5%, #0088FF 30%, #00D2FF 70%, transparent 95%);font-size:0;line-height:0;">&nbsp;</td>
          </tr>

          <!-- Header Branding -->
          <tr>
            <td style="padding:26px 32px 18px;border-bottom:1px solid rgba(255,255,255,0.06);">
              <table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0">
                <tr>
                  <td align="left">
                    <a href="https://adamsxproject.com.ng" target="_blank" style="text-decoration:none;display:inline-flex;align-items:center;">
                      <img src="https://adamsxproject.com.ng/favicon-32x32.png" width="20" height="20" alt="Logo" style="border-radius:5px;vertical-align:middle;display:inline-block;margin-right:8px;" />
                      <span style="font-size:12px;font-weight:800;letter-spacing:0.14em;color:#FFFFFF;text-transform:uppercase;vertical-align:middle;">// ADAMS X PROJECT</span>
                    </a>
                  </td>
                  <td align="right">
                    <span style="background:#091530;border:1px solid rgba(0,210,255,0.32);color:#38BDF8;font-size:9.5px;font-weight:700;letter-spacing:0.16em;text-transform:uppercase;padding:4px 10px;border-radius:100px;display:inline-block;">${kicker}</span>
                  </td>
                </tr>
              </table>
            </td>
          ...
          <!-- Main Content Body -->
          <tr>
            <td style="padding:32px 32px 24px;color:#E2E8F0;font-size:15px;line-height:1.75;">
${body}
            </td>
          </tr>
          ...
          <!-- Footer -->
          <tr>
            <td style="padding:22px 32px 28px;background:#040914;border-top:1px solid rgba(255,255,255,0.06);text-align:center;">
              <p style="font-size:11.5px;color:#64748B;margin:0 0 8px;line-height:1.5;">
                Adams X Project &middot; Comeback: Unrecognizable Protocol &middot; Lagos, NG
              </p>
              <p style="font-size:11.5px;color:#64748B;margin:0;">
                <a href="https://adamsxproject.com.ng/privacy" style="color:#94A3B8;text-decoration:underline;margin-right:12px;">Privacy Policy</a>
...
function ln(text) {
    return `      <p style="margin:0 0 18px;font-size:15px;line-height:1.75;color:#CBD5E1;">${text}</p>\n`;
}

// Helper — sign-off line
function signOff(name = '— Adams X') {
    return `      <p style="margin:22px 0 0;font-size:15px;font-weight:700;color:#FFFFFF;">${name}</p>\n`;
}

// Helper — PS line (Glass callout)
function ps(text) {
    return `      <div style="margin:22px 0 0;padding:14px 18px;background:rgba(0,210,255,0.05);border-left:3px solid #00D2FF;border-radius:4px;"><p style="margin:0;font-size:13.5px;line-height:1.6;color:#94A3B8;"><strong style="color:#38BDF8;">P.S.</strong> ${text}</p></div>\n`;
}

// Helper — link
function link(url, label) {
    return `<a href="${url}" style="color:#38BDF8;font-weight:600;text-decoration:underline;">${label || url}</a>`;
}
...
// Helper — Cyan accent highlight
function cyan(text) {
    return `<span style="color:#38BDF8;font-weight:700;">${text}</span>`;
}

// Helper — Bento Feature Box
function bentoBox(title, items = []) {
    const listHtml = items.map(item => `<p style="margin:0 0 8px;font-size:13.5px;color:#E2E8F0;line-height:1.5;">✔ ${item}</p>`).join('');
    return `      <table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0" style="background:#09142A;border:1px solid rgba(0,210,255,0.25);border-radius:12px;padding:18px 20px;margin:22px 0;">
        <tr>
          <td>
            <p style="margin:0 0 10px;font-size:11.5px;font-weight:800;letter-spacing:0.12em;color:#38BDF8;text-transform:uppercase;">⚡ ${title}</p>
            ${listHtml}
          </td>
        </tr>
      </table>\n`;
...
// Helper — Testimonial Glass Box
function testimonialBox(quote, author) {
    return `      <table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0" style="background:rgba(6,15,32,0.85);border:1px solid rgba(0,210,255,0.2);border-left:3px solid #00D2FF;border-radius:8px;padding:14px 18px;margin:18px 0;">
        <tr>
          <td>
            <p style="margin:0 0 6px;font-size:13.5px;line-height:1.6;color:#CBD5E1;font-style:italic;">"${quote}"</p>
            <p style="margin:0;font-size:12px;font-weight:700;color:#38BDF8;">— ${author}</p>
          </td>
        </tr>
      </table>\n`;
}

// Helper — Executive CTA Button
function ctaButton(url, label, isGold = true) {
    const bg = isGold 
        ? 'background:linear-gradient(135deg, #FFE600 0%, #F5CE00 100%);color:#000000;box-shadow:0 6px 20px rgba(255,230,0,0.32);'
        : 'background:linear-gradient(135deg, #00D2FF 0%, #0088FF 100%);color:#000000;box-shadow:0 6px 20px rgba(0,210,255,0.32);';
    return `      <p style="margin:26px 0;text-align:center;"><a href="${url}" target="_blank" style="display:inline-block;${bg}font-size:13.5px;font-weight:800;letter-spacing:0.06em;text-transform:uppercase;text-decoration:none;padding:15px 36px;border-radius:100px;border:1px solid rgba(255,255,255,0.5);">${label}</a></p>\n`;
}
