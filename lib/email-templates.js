        ...
        html: emailLayout({
            ...
            bodyHtml: `
                ...
                <div style="background:#111111;border:1px solid #1A1A1A;padding:36px 28px;margin:28px 0;text-align:center;">
                    ...
                    <p style="font-family:Arial,Helvetica,sans-serif;margin:0 0 24px;line-height:1.2;text-align:center;">
                        <span style="font-family:Georgia,'Times New Roman',serif;font-size:20px;color:#888888;text-decoration:line-through;text-decoration-color:rgba(201,168,76,0.5);margin-right:8px;vertical-align:middle;">$67</span>
                        <span style="font-family:Arial,Helvetica,sans-serif;font-size:16px;color:#888888;margin-right:8px;vertical-align:middle;opacity:0.5;">&rarr;</span>
                        <span style="font-family:Georgia,'Times New Roman',serif;font-size:32px;font-weight:700;color:#C9A84C;vertical-align:middle;text-shadow:0 0 20px rgba(201,168,76,0.15);">$17</span>
                        <span style="display:inline-block;background:rgba(201,168,76,0.1);border:1px solid rgba(201,168,76,0.25);color:#C9A84C;font-family:Arial,Helvetica,sans-serif;font-size:10px;font-weight:700;letter-spacing:0.1em;text-transform:uppercase;padding:4px 10px;border-radius:100px;margin-left:8px;vertical-align:middle;">Launch Offer</span>
                    </p>
        ...
        html: emailLayout({
            ...
            bodyHtml: `
                ...
                <div style="background:#111111;border:1px solid #1A1A1A;padding:36px 28px;margin:28px 0;text-align:center;">
                    ...
                    <a href="${WAITLIST_URL}" style="display:inline-block;background:#C9A84C;color:#0A0A0A;font-family:Arial,Helvetica,sans-serif;font-size:14px;font-weight:700;text-transform:uppercase;letter-spacing:0.15em;padding:16px 36px;text-decoration:none;border-radius:2px;">Get Access Now &mdash; $17 Launch Offer</a>
                </div>
...
<div style="max-width:620px;margin:0 auto;background-color:#0A0A0A;padding:32px 24px;">
  ...
  // ============================================================
// DAY 10 — Still Thinking (follow up for non-buyers)
// ============================================================
function day10(firstName, email) {
    return {
        ...
        html: emailLayout({
            dayLabel: 'FOLLOW UP',
            headline: 'You Felt the Shift.',
            email,
            bodyHtml: `
                ${p(`${firstName}, you finished the 7 days.`)}
                ${p(`You felt something shift. I know you did — because the people who make it to Day 7 always do.`)}
                ${p(`The full blueprint is still waiting for you. <strong>Comeback: Unrecognizable</strong> — the complete 90-day protocol. The 7 rules. The exact system I use to build while everyone around me is still talking about it.`)}
                ${p(`It is only <span style="text-decoration:line-through;color:#888888;">$67</span> &rarr; <span style="color:#C9A84C;font-weight:700;font-size:16px;">$17 Launch Offer</span>. That price will not stay forever.`)}
...
// ============================================================
// DAY 14 — This Is Different (follow up for non-buyers)
// ============================================================
function day14(firstName, email) {
    return {
        ...
        html: emailLayout({
            ...
            email,
            bodyHtml: `
                ${p(`${firstName}, most courses give you information.`)}
                ${p(`<strong>Comeback: Unrecognizable</strong> gives you an operating system.`)}
                ${p(`90 days. 7 rules. The exact playbook I used to build while everyone around me was still waiting for the right moment. The right moment was never coming. So I built the system instead.`)}
                ${p(`<span style="text-decoration:line-through;color:#888888;">$67</span> &rarr; <span style="color:#C9A84C;font-weight:700;font-size:16px;">$17 Launch Offer</span>. Everything I know. Everything I apply. Documented and structured for you to follow.`)}
                ${ctaButton('Get the Full Blueprint — $17 Launch Offer', WAITLIST_URL)}
                ${signOff()}
            `
        })
...
// ============================================================
// DAY 21 — Last Reminder (final follow up for non-buyers)
// ============================================================
function day21(firstName, email) {
    return {
        subject: `Last time I'll mention this, ${firstName}`,
        html: emailLayout({
            dayLabel: 'FINAL FOLLOW UP',
            headline: 'Last Call.',
            email,
            bodyHtml: `
                ...
                ${p(`This is the last time I will mention it. After this email I will respect your decision either way.`)}
                ${p(`<strong>Comeback: Unrecognizable — <span style="text-decoration:line-through;color:#888888;font-weight:400;">$67</span> &rarr; <span style="color:#C9A84C;">$17 Launch Offer</span>.</strong> The complete 90-day system.`)}
                ${ctaButton('Get It Now — $17 Launch Offer', WAITLIST_URL)}
                ${pMuted(`Whatever you decide — I am still rooting for you.`)}
                ${signOffFull()}
            `
        })
    };
...
// ============================================================
// APOLOGY RESEND — Updated Starter Kit Link
// ============================================================
function apologyResend(firstName, email) {
    ...
    return {
        ...
        html: emailLayout({
            dayLabel: 'IMPORTANT UPDATE',
            headline: 'Your Download Link Has Been Updated.',
            email,
            bodyHtml: `
                ${p(`Hey <strong>${firstName}</strong> — sorry about this.`)}

                ${p(`When you first signed up, the download link in your welcome email wasn't working correctly. I've fixed it and wanted to make sure you actually get the kit you signed up for.`)}

                ${p(`Your <strong>7-Day Monk Mode Starter Kit</strong> is ready and waiting. Click below to download it now:`)}

                <!-- EBOOK PDF: Tracked download -->
                ${ctaButton('Download Your Starter Kit', downloadUrl)}

                ${p(`The 7-day email sequence continues as normal. If you missed any earlier emails, keep an eye on your inbox — they'll keep coming.`)}

                ${p(`Again, sorry for the friction. Now you have it. — <em>Adams</em>`)}
            `
        })
    };
}
...
function paidEbookAccess(firstName, downloadUrl, email) {
    ...
    return {
        ...
        html: emailLayout({
            dayLabel: 'PRODUCT DELIVERY',
            headline: `Access Granted, ${firstName}.`,
            email,
            bodyHtml: `
                ${p(`You just made the move most people only think about. <strong>Comeback: Unrecognizable</strong> is your new operating system.`)}

                ${p(`This is the complete 90-day system. No motivation speeches. No theory. Just the system. You have taken sovereign ownership of your focus and income path.`)}

                <div style="background:#111111;border:1px solid #1A1A1A;padding:30px 24px;margin:24px 0;">
                    <p style="font-family:Georgia,'Times New Roman',serif;font-size:15px;color:#C9A84C;font-weight:700;margin:0 0 14px;text-transform:uppercase;letter-spacing:0.1em;">What You Now Have Access To:</p>
                    <ul style="margin:0;padding-left:20px;font-family:Arial,sans-serif;font-size:14px;line-height:1.7;color:#E8E8E8;">
                        <li style="margin-bottom:8px;"><strong>The 7 Core Rules</strong> — The rules that will make people ask what changed about you.</li>
                        <li style="margin-bottom:8px;"><strong>The 90-Day Protocol</strong> — The exact daily execution blueprint broken into phases.</li>
                        <li style="margin-bottom:8px;"><strong>The Builder's Stack</strong> — Playbooks and setups to build a $10k/month income from zero.</li>
                        <li style="margin-bottom:8px;"><strong>Secure Downloads</strong> — Future system guides, logs, and printable trackers.</li>
                    </ul>
                </div>

                ${ctaButton('Access Your Product', accessUrl)}

                ${p(`This is the work. Show up every single day. Do not overthink it.`)}
...
// ============================================================
// BONUS DELIVERY — The Secret Bonus Gift
// ============================================================
function bonusDelivery(firstName, email) {
    // Replace the link below with your actual bonus content (e.g., Notion template, additional PDF, etc.)
    const bonusLink = 'https://adamsxproject.com.ng/ebooks/the-7-day-starter-kit.pdf'; 
    return {
        subject: `🎁 Secret Bonus Gift Unlocked, ${firstName}`,
        html: emailLayout({
            dayLabel: 'BONUS UNLOCKED',
            headline: `You Finished the Kit, ${firstName}.`,
            email,
            bodyHtml: `
                ${p(`Congratulations. You actually read the 7-Day Monk Mode Starter Kit and found the secret link. Most people consume headlines; you consume systems. That is a massive advantage.`)}

                ${p(`As promised, here is your <strong>Secret Bonus Gift</strong> — the physical focus audit templates, habit log Notion integration, and the distraction blocker script configuration.`)}

                ${ctaButton('Access Your Bonus Gift', bonusLink)}

                ${p(`Use it to build your workspace sovereignty. The 7-day email sequence is still running in the background. Tomorrow's protocol email will arrive at 9:00 AM UTC.`)}

                ${p(`Keep building. I'm rooting for you. — <em>Adams</em>`)}
            `
        })
    };
}
